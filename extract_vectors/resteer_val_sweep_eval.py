...
def main() -> None:
    ...
    (
        ...
        out_png_avg,
        vectors_results_tag,
    ) = resolve_result_paths(Path(args.vectors), args.direction)
    print(
        f"Outputs → {out_sweep_json.parent} "
        f"(direction={args.direction}, vectors tag={vectors_results_tag})",
        flush=True,
    )
