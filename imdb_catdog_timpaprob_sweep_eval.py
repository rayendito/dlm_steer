#!/usr/bin/env python3
"""Evaluate every IMDb and Cat/Dog TIMPA-probabilistic sweep trial."""
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all IMDb and Cat/Dog TIMPA-probabilistic sweep trials."
        )
    )
    parser.add_argument(
        "--overwrite",
...
def _find_trial_directories(dataset_name, directions):
    dataset_root = RESULTS_ROOT / f"{dataset_name}_sweep_csv"
    if not dataset_root.is_dir():
        raise FileNotFoundError(
            f"Sweep directory does not exist: {dataset_root}"
        )
    ...
    with tqdm(
        ...
        for dataset_name, (
            ...
            for trial_directory in trial_directories:
                ...
                items = _read_trial(trial_directory, directions)
                steering_successes, target_probability_gains = _steering_metrics(
                    items=items,
                ...
                existing_judge_scores = _read_existing_judge_scores(
                    output_path,
                    expected_rows,
                )
                if existing_judge_scores is None:
                    faithfulness_scores, structure_scores = _judge_scores(items)
                else:
                    faithfulness_scores, structure_scores = existing_judge_scores
                _write_evaluation(
                    output_path=output_path,
                    items=items,
                    steering_successes=steering_successes,
                    target_probability_gains=target_probability_gains,
